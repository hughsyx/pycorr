...
def reader(file_data='', patch_shape=2048, decimation=0, channels=None,
           trim=None, extraction_step=None, batch_size=None, **kwargs):
    """Read seismic data.
    ...
    def h5read(self, path_h5, net='PZ', force_start=None, stations=None,
        ...
        for station, station_code in enumerate(station_codes):
            ...
            try:
                ...
                if 'sta' in h5file['_metadata']:
                    stats.coordinates = AttribDict()
                    stats.coordinates.latitude = h5file['_metadata']['sta'][net][station_code]['lat'][()]
                    stats.coordinates.longitude = h5file['_metadata']['sta'][net][station_code]['lon'][()]
                    if 'ev' in h5file['_metadata']:
                        stats.ev_coordinates = AttribDict()
                        stats.ev_coordinates.latitude = h5file['_metadata']['ev']['lat'][()]
                        stats.ev_coordinates.longitude = h5file['_metadata']['ev']['lon'][()]
                        [dist,az,baz] = gps2dist(stats.coordinates.latitude,stats.coordinates.longitude,
                            stats.ev_coordinates.latitude ,stats.ev_coordinates.longitude)
                        stats.distance = degrees2kilometers(dist)
