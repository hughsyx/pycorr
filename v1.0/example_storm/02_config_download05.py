...
in_['ch']             = ['BH','HH','SH','EH'] # ['ch1','ch2',...] It will try in this order. Better to start from the lower sampling rate ['LH','BH','HH'...]
in_['cmp']            = ['Z','E','N'] # ['cmp1','cmp2',...]
in_['tag']            = 'glob_05' # subdirectory name for the output /data_*freq*Hz/dauly/**tag**/**year**/
in_['station_list']   = 'stations05.txt' # input station file from 00_get_stations.py or other
in_['path_out']       = './' # output path

############### conventional preprocessing

in_['pp']                = {} # always '{}'
in_['pp']['freq']        = 4. # [hz], freq. to which data are decimated [hz] => output sampling rate
in_['pp']['f_prefilt']   = (0.005, in_['pp']['freq'] / 2 - 0.10 * in_['pp']['freq']) # [hz], (f0,f1) used for butter-BP filter after sensor response deconvolution
