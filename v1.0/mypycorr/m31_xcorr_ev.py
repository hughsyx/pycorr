...
# June 2019
#modified by Lisa Tomasetto in February 2022
################################################
...
def xcorr_ev(inu, db, fe):
    ...
    ex = {}
    ex['set'] = {}
    ex['out_dir'] = in_['path_out'] + '/C1_' + caller + '_' + in_[
        'tag']  # +'_'+in_['cc_func'][4:-1]+in_['cc_func'][-1]  #to be changed later
    for ipp in in_['pp']:
        ex['out_dir'] += '_' + ipp
...
def correlate_this_set_ev(in_, md_c, kset):
    ...
    for kdate in ev_list:
        ...
        stra = kset['path1']+'/*.h5'
        h5a_name = glob.glob(stra)
        h5a_name.sort()
        strb = kset['path2']+'/*.h5'
        h5b_name = glob.glob(strb)
        h5b_name.sort()
        ...
        for icmp, kcmp in enumerate(in_['cc_cmp']):
            ...
            for icpl, kcpl in enumerate(kset['md']['id']):
                ...
                # add for events with multiples download files for data
                ff = open(kset['path1']+'sumup.txt','r')  # sumup tells in which file is the station's data
                lines = ff.readlines()
                ...
                try:
                    ...
                    if in_['pp']:
                        ...
                        dd.dispc(
                            '                         : ' + _cts_get_current_hr_as_str() + ' : pre processing 2nd set',
                            'c', 'n')
                    ...
                    if in_['event_stack']:
                        ...
                        if in_['pws']:
                            h5_create_dataset(h5_daily, '/cc' + dset_name,
                                              pws(cc_hr, in_['pws_timegate'], in_['pws_power'], 1 / float(md_c['tau']),
                                                  in_['cc_dtype']), in_['gzip'], in_['cc_dtype'])
                        elif in_['svd_wiener2']:
                            h5_create_dataset(h5_daily, '/cc' + dset_name,
                                              svd_wiener2(cc_hr, in_['svd_wiener2_m'], in_['svd_wiener2_n'],
                                                          in_['svd_wiener2_nvs'], in_['cc_dtype']), in_['gzip'],
                                              in_['cc_dtype'])
                        else:
                            h5_create_dataset(h5_daily, '/cc' + dset_name, cc_hr.sum(axis=0), in_['gzip'],
                                              in_['cc_dtype'])
...
def ex_determine_npath_per_h5_file_ev(in_, fe):
    ...
    if in_['cc_dtype'] == 'float64':
        size_single_path = size_single_path * 8.0
    elif in_['cc_dtype'] == 'float32':
        size_single_path = size_single_path * 4.0
    elif in_['cc_dtype'] == 'float16':
...
def ml_stack_and_write_this_set_in_hdf5_ev(in_, md_c, kset):
    ...
    for ipath in range(0, ncpl):
        ...
        for icmp in range(0, ncmp):
            ...
            dset_name = h5_get_station_pair_tree_path(kset['md']['id'][ipath], in_['cc_cmp'][icmp])
            if in_['pws']:
                stack_data = pws(ff_cc['/cc' + dset_name], in_['pws_timegate'], in_['pws_power'],
                                 1 / float(md_c['tau']), in_['cc_dtype'])
            elif in_['svd_wiener2']:
                stack_data = svd_wiener2(ff_cc['/cc' + dset_name], in_['svd_wiener2_m'], in_['svd_wiener2_n'],
                                         in_['svd_wiener2_nvs'], in_['cc_dtype'])
            else:
