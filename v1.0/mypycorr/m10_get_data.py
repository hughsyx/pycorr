...
def download(input_user={}): 
    ...
    for idate in list_of_dates :
        ...
        md   = core_test(in_,db,md,path,cday,cday+86400.0)        
...
def download_sta(input_user={}): 
    ''' download events, slicing data files per number of stations given by user.'''
    in_         = {} 
    in_['path'] = './data_test/events/tag/'
    in_['mode'] = 0  # 0=all which has not yet been downloaded 1=timeout 2=timeout+incorrect  
    in_['nb_sta'] = 50  # number of stations per download file, will determine the number of files
    in_         = lang.parse_options(in_,input_user)
    ...
    try :
        ...
        dd.dispc('You need to configure first !','r','b')
        return
    in_['nb_files'] = len(db['sta'])//in_['nb_sta'] + 1  # Number of download files     
    for iev in db['ev']:
        ...
        for files in path.keys():
            ...
            db_slice = {}
            db_slice['in_'] = db['in_']
            db_slice['sta'] = dict(list(db['sta'].items())[(n*in_['nb_sta']):(n+1)*in_['nb_sta']])
            db_slice['ev'] = db['ev']
            
            md   = load_metadata(path[files]['metadata_pkl'])
            md   = core_download(in_,db_slice,md,path[files],date1,date2)
            #exiting this day  : write metadata + remove lock file + create a _day_001.done file
            try :
                write_metadata_hdf5(path[files]['h5_file'],db_slice['in_'],date1,sta=db_slice['sta'],ev=db_slice['ev'][iev])
            except  : 
                pass
            filename = path[files]['metadata_pkl']
            ff       = open(path[files]['metadata_pkl'],'wb')
            pickle.dump(md,ff)
            ff.close()
            io.savemat(path[files]['metadata_mat'],md)
            os.remove(path[files]['h5_lock_file'])
            if in_['mode'] !=0 :  #pour la gestion du multicore on est oblige de creer un ficher factice
                create_lock_file(path[files]['h5_finished'])
                # exiting this download session :
            if in_['mode'] >0 : # if we generated *_done_files => rm them if there is not *cplt file (code weakness)
                delete_done_files_if_no_more_cplt(in_['path'])
...
def core_test(in_,db,md,path,date1,date2): 
    ''' 
    loop over station list and channel for a given range of dates...
    '''
    for ista in db['sta'].values() :
        ...
        for icmp in db['in_']['cmp'] :
            ...
            result   = queue.get()"""
            time_out = False
            result = load_test(db['in_'], path['h5_file'], client, date1, date2, ista_arg, icmp)
            new_ista = result[0]  # if not timoeout : get updated station info with locid and channel (BH,HH)
...
def load_test(in_,out_file,client,date1,date2,ista,icmp) :
    '''download and decimate/rm instrumental response of a single day/station/cmp and write in a h5 file
    '''
    tap_len     = in_['pp']['tap_len']
    remove_resp = in_['pp']['remove_resp']
...
#------------------------------------------
def define_path_sta(in_,ev_id) : 
    n = in_['nb_files']
    path                = {}
    for n in range(in_['nb_files']):
        path['file%d'%n] = {}
        path['file%d'%n]['out_dir']     = in_['path']
        path['file%d'%n]['h5_file']     = path['file%d'%n]['out_dir']+'/'+ev_id+'_%03d'%n+'.h5'
        path['file%d'%n]['h5_finished'] = path['file%d'%n]['out_dir']+'/_'+ev_id+'_%03d'%n+'.done'
        if in_['mode']==0 :
