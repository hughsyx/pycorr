...
def _main_loop(in_dir,out_dir,flist,in_) :
    ...
    for iyear in year_list :
        ...
        #update the fe in db files if we are decimating the data: 
        if '_decimate' in in_['recipe'] :
            #read fe from input db.pkl :
            db_open = open(iyear+'/db.pkl', "rb" )
            dbin = pickle.load(db_open)
            fe=dbin['in_']['pp']['freq']
            db_open.close()
            # compute the new fe :
            I_decimate = in_['recipe'].index('_decimate')
            dbin['in_']['pp']['freq'] = fe / in_['args'][I_decimate]['factor'] 
            # write it to the output directory db file :
            db_open= open(out_year+'/db.pkl', "wb" )
            pickle.dump(dbin, db_open)
            db_open.close()
            # check the result
            #db_open = open(iyear+'/db.pkl', "rb" )
            #dbin = pickle.load(db_open)
            #print(dbin['in_']['pp']['freq'])
            #db_open.close()
        ...
        for ih5 in h5_list : 
            ...
            #update fe if we decimate the trace
            if '_decimate' in in_['recipe'] :
                if 'fe' in fout['/_metadata'] : # just in case the file is empty
                    I_decimate = in_['recipe'].index('_decimate')
                    new_fe = np.float(np.array(fout['/_metadata/fe'])) / in_['args'][I_decimate]['factor'] 
                    del fout['/_metadata/fe']
                    fout.create_dataset('/_metadata/fe',data = new_fe)
                    ...
                    for ista in sta_list :
                        ...
                        #for ich in ista[1].items() :
                            ...
                            for fh in flist:
                                ...
                                # update fe if we used the get_enveloppe method ?
...
#-------------------------------------------
def _decimate(trace,fe,inu={}) :
    in_ = {}
    in_['factor'] = 5
    in_ = __check_in(in_,inu,'_decimate')

    trace = signal.decimate(trace,int(in_['factor']),zero_phase=True)
    return trace, in_
