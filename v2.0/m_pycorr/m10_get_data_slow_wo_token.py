    ...
    for ista in db['sta'].values() :
        ...
        for icmp in cmp_list : #db['in_']['cmp'] :
            ...
            if data_center == 'SDS':
                ...
                ###################
                client = initialize_client(data_center,db['in_'],in_['token'],in_['user'],in_['pwd'])
                if client == False : 
                    md[ista['kname']][icmp]['reached_datacenter'] = False 
                    md[ista['kname']][icmp]['output']             = 'could not initialize the client'
                    md[ista['kname']][icmp]['success']            = False 
            ...
            if time_out== False :            
                if type(result[1]) == dict : # can be False if the code crashed for some reasons (unable to open the .h5)
                    md[ista['kname']][icmp]             = result[1]
                    md[ista['kname']][icmp]['time_out'] = False
                else :
                    md[ista['kname']][icmp]={}
                    md[ista['kname']][icmp]['output']='OK'
...
def initialize_client(data_center,db,token_path,user,pwd) :
    ...
    try :
        client = obspy.clients.fdsn.Client(data_center,user=user,password=pwd)   
...
def has_this_day_already_been_processed(mode,path) :
    ...
    if done == False : dd.dispc('  '+datetime.now().strftime('%Y_%m_%d__%Hh%M:%S')+' : '+path['h5_file'] + ': lets go','y','b')
    return done 
