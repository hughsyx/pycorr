...
#import m_pycorr.mods.cc_h5 as cc 
import m_pycorr_dev.mods.dd as dd 
...
def process_this_group(igroup, path_list,fh,in_fh,in_):
	...
	for ipath in path_list_random  : 
		...
		dd.dispc('        '+str(dt.now())[0:-7]+'  '+pydb_path['root'],'c','n')
...
def process_these_paths(c1_name,kpath,fh,in_fh,in_) : 
	...
	for icmp in cmp_list : 
		...
		for ipath in range(kpath['I1'],kpath['I2']+1,1) :
			id_=ff['/md/id'][ipath]
			path_in_h5='/'+id_[0].decode('UTF8')+'/'+id_[1].decode('UTF8')+'/'+icmp
			...
			id_tmp = ff['/md/id'][ipath]
			data['id'] = [id_tmp[0].decode('UTF8'), id_tmp[1].decode('UTF8')]
			result_for_this_path, md_c, in_fh_used =fh(data,in_fh)
			result[icmp].append(result_for_this_path)
			if icmp == cmp_list[0]:
				result['h5_path'].append('/'+ff['/md/id'][ipath][0].decode('UTF8')+'/'+ff['/md/id'][ipath][1].decode('UTF8'))
	ff.close()
	result['md_c']  = md_c
...
def add_metadata_from_xcorr_file(pydb_file, c1_list) :
	...
	md_list_tmp = fx['/md'].keys()
	md_list = [] 
	for imd in md_list_tmp : 
		md_list.append(imd) 
...
def concatenate(filenames,tab=4,prefix='') :
	...
	for ih5 in h5_list :
		fh5 = h5py.File(ih5,'r')
		key_list_tmp =fh5.keys()
		key_list =[] 
		for ikey in key_list_tmp : 
...
def h5_save_dict_keys_as_group(h5,dict_,prefix=''):
    for ikey in dict_ :
        ...
        for dname, dset in dict_[ikey].items():
            ...
            # list of string : 
            elif type(dset)==list and len(dset) >0 and type(dset[0])==str :
                h5.create_dataset(prefix+'/'+ikey+'/'+dname,data=np.array(dset,dtype='S'))
            else :
