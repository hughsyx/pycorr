...
classdef c3_live_md < handle 
    ...
    %---------------------------------------
    methods 
        function p_vs_map(h,varargin)
            in.topo= false ;
            in.p_sta= true; 
            in.p_text= false;
            in = lang.parse_options(in,varargin); 
            
            lat=[min(h.vs.lat) max(h.vs.lat)];
            lon=[min(h.vs.lon) max(h.vs.lon)];
            dy = (lat(2)-lat(1))/5; 
            dx = (lon(2)-lon(1))/5; 
            map.background([lon(1)-dx,lon(2)+dx],[lat(1)-dy,lat(2)+dy],'topo',in.topo);
        
            if in.p_sta 
                map.station(h.lon,h.lat);
            end
            
            if in.p_text 
                for ista = 1 : numel(h.vs.lon); 
                    m_text(h.vs.lon(ista), h.vs.lat(ista),num2str(ista));
                end
            end
        end
        ...
        function p_map(h,in_map,in_background,in_sta)
            ...
            sta.sta = sta.sta(I);
            sta.elev= sta.elev(I);
            dx = [max(sta.lon) - min(sta.lon)]/5; 
            dy = [max(sta.lat) - min(sta.lat)]/5; 
            in.lon(1) = min(sta.lon)-dx;
            in.lon(2) = max(sta.lon)+dx; 
            in.lat(1) = min(sta.lat)-dy;
            in.lat(2) = max(sta.lat)+dy;
