...
class c1_md : 
    ...
    def read_c1(self,I_path,I_cmp=0,I_date=-1,reverse=False)  : 
        ...
        try :
            ...
            dset_name = dset_name + '/' + ccmp.decode()
        tr={}
        ...
        try :
            tr['title'] = self.id[I_path][I[0]]+'-'+self.id[I_path][I[1]]+' '+ccmp.decode()
            tr['cmp']   = ccmp.decode()
        except : 
            tr['title'] = self.id[I_path][I[0]]+'-'+self.id[I_path][I[1]]
            tr['cmp']   = ccmp
...
def build_db_file(file_list) : 
    ...
    # on perd du temps avec une conversion bytes -> string, merci python ...
    k=-1
    cc_cmp=[]
    for icmp in db['in_']['cc_cmp'] :
        k=k+1
        cc_cmp.append(icmp.decode('utf-8'))
        db['in_']['cc_cmp'] = np.array(cc_cmp)
